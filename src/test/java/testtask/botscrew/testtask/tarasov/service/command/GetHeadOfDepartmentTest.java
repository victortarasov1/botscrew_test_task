...
class GetHeadOfDepartmentTest {
    private DepartmentRepository departmentRepository;
    private Command command;
    private final static String VALID_QUERY = "Who is head of department Mathematics";
    private final static String INVALID_QUERY = "Who are heads of department Mathematics";

    @BeforeEach
    public void setUp() {
        departmentRepository = mock(DepartmentRepository.class);
        command = new GetHeadOfDepartment(departmentRepository);
    }
    ...
    public void testExecute() {
        ...
        when(departmentRepository.findDepartmentAndHeadByName(anyString())).thenReturn(Optional.of(department1));

        command.execute(VALID_QUERY);

        verify(departmentRepository, times(1)).findDepartmentAndHeadByName(anyString());
